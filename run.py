...
def run_sequential(args, logger):
    ...
    # Learner
    learner = le_REGISTRY[args.learner](mac, buffer.scheme, logger, args)  # todo
    ...
    while runner.t_env <= args.t_max:
        ...
        # 生成 batch of episode 并存入 buffer
        # Run for a whole episode at a time
        episode_batch = runner.run(test_mode=False)
        buffer.insert_episode_batch(episode_batch)

        if buffer.can_sample(args.batch_size):   # buffer 够 sample 一个 batch
            episode_sample = buffer.sample(args.batch_size)

            # Truncate batch to only filled timesteps
            max_ep_t = episode_sample.max_t_filled()    # 这个 batch 里的所有回合里 timesteps 的最大值
            episode_sample = episode_sample[:, :max_ep_t]   # 截断

            if episode_sample.device != args.device:
                episode_sample.to(args.device)
            # 训练，传入 batch, 已跑了的 timesteps，已从 buffer 中取了的回合数
