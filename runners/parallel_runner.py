...
class ParallelRunner:
    ...
    def run(self, test_mode=False):
        ...
        if not test_mode:
            self.t_env += self.env_steps_this_run   # 加上当前 timestep 下未终止的环境的数量， t_env 是 total_timesteps
...
def env_worker(remote, env_fn):
    ...
    while True:
        ...
        if cmd == "step":
            ...
            state = env_utils.get_state(env)
            avail_actions = env_utils.get_avail_actions(env)
            obs = env_utils.get_obs(env)
            remote.send({
            ...
            remote.send(env_utils.get_env_info())
        elif cmd == "get_stats":
