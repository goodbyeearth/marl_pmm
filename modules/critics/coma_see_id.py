...
class SeeIDCritic(nn.Module):
    def __init__(self, scheme, args):
        super(SeeIDCritic, self).__init__()
        ...
        self.fc4 = nn.Linear(16+input_shape['id_shape'], self.n_actions)
    ...
    def forward(self, batch, t=None):
        ...
        x = F.relu(self.fc2(x))
        x = F.relu(self.fc3(x))
        x = th.cat([x, inputs['id_inputs']], dim=1)
        q = self.fc4(x)
    ...
    def _build_inputs(self, batch, t=None):
        ...
        inputs['id_inputs'] = th.eye(self.n_agents, device=batch.device)\
            .unsqueeze(0).unsqueeze(0).expand(bs, max_t, -1, -1).reshape(bs*max_t*self.n_agents, -1)
